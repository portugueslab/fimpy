"""
This module is copypasted from current scikit-image master after Vilim's upgrade
for 3D registration support. Can be removed once the new scikit-image version including
...
def _upsampled_dft(data, upsampled_region_size, upsample_factor=1, axis_offsets=None):
    ...
    if data.ndim == 2:
        ...
        try:
            return np.einsum(
                "ijk, li, mj, nk -> lmn", data, *dim_kernels, optimize=True
            )
        except TypeError:
            warnings.warn(
                "Subpixel registration of 3D images will be very slow"
        ...
        raise NotImplementedError(
            "Upsampled registration for images of more"
